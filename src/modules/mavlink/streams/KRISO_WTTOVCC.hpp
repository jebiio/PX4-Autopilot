#ifndef KRISO_WTTOVCC_HPP
#define KRISO_WTTOVCC_HPP

#include <uORB/topics/kriso_wttovcc.h>


class MavlinkStreamKrisoWtToVcc : public MavlinkStream
{
public:
	static MavlinkStream *new_instance(Mavlink *mavlink) { return new MavlinkStreamKrisoWtToVcc(mavlink); }

	static constexpr const char *get_name_static() { return "WTTOVCC"; }
...
private:
	explicit MavlinkStreamKrisoWtToVcc(Mavlink *mavlink) : MavlinkStream(mavlink) {}

	uORB::Subscription _home_sub{ORB_ID(kriso_wttovcc)};

	bool send() override
	{
		// we're sending the GPS home periodically to ensure the
		// the GCS does pick it up at one point
		kriso_wttovcc_s home;
